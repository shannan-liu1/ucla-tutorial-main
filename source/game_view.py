...
from glow_line import GlowLine # task 4: import glow line
...
class GameView(arcade.View):
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        # task 2: when the player shoots, play laser sound
    ...
    def fire_projectile(self, laser_color):
        ...
        # task 4: change glowball to glowline
        # and use the glowline shadertoy
        ...
        for laser in self.laser_list:
            ...
            for target in targets_hit:
                if colors_match(laser.get_color(), target.get_color()):
                    ...
                    # task 2: when target is hit, play the hit sound 
                    arcade.play_sound(self.hit_sound) 

                    self.target_manager.remove_target(target)

                    # task 1: customise explosion color to match
                    # color of the orb
                    explosion = ExplosionMaker(
                        self.window.get_size(), target.position,
                        color=target.get_color()
                    )
                    self.explosion_list.append(explosion)

                    # task 3: when target is hit & colors match, increment score
                    self.score += 1
    ...
    def on_draw(self):
        ...
        # task 3: show the player their score
